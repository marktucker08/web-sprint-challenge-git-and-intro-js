...
function addArtist(arr, nameStr, yearsStr, genreStr, nationalityStr, bioStr) {
  arr.push({
    "name": nameStr,
    "years": yearsStr,
    "genre": genreStr,
...
console.log(addArtist(artists, 'John Denver', '1960-1988', 'Full Stack Development', 'African American', 'I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.'));
...
function lotsOfArt(arr) {
  const lotOfPaintings = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].paintings > 100) {
      lotOfPaintings.push(arr[i].name);
    }
  }
  return lotOfPaintings;
}
...
console.log(artistInfo(artists, "Gustav Klimt"));
...
function artistByCountry(arr, nationalityStr){
  const artistsNationality = []; 
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].nationality === nationalityStr) {
      artistsNationality.push(arr[i].name);
    }
  }
